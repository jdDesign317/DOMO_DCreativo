...
// ORDENAMIENTO
$columnas_permitidas = ["id_tipo_evento", "nombre", "activo"];
...
$url_base = "index.php?accion=tipos_evento&orden={$orden}&dir={$direccion}&buscar=" . urlencode($buscar);
?>

<h4 class="text-domocreativo mb-3">Tipos de Evento</h4>

<div class="mb-3">
    <a href="index.php" class="btn btn-secondary">Volver</a>
    <a href="index.php?accion=tipos_evento_crear" class="btn btn-domocreativo">
...
<form method="GET" action="index.php" class="mb-3">
    <input type="hidden" name="accion" value="tipos_evento">
    ...
    <div class="input-group" style="max-width:400px;">
        ...
        <?php if ($buscar !== "") : ?>
            <a href="index.php?accion=tipos_evento" class="btn btn-outline-secondary">✕ Limpiar</a>
        <?php endif; ?>
    </div>
</form>
...
<?php if ($total === 0) : ?>
    <div class="alert alert-info">
        <?= $buscar !== "" ? "No se encontraron tipos de evento para \"" . htmlspecialchars($buscar) . "\"." : "No hay tipos de evento registrados." ?>
    </div>
<?php else : ?>
...
<table class="table table-bordered table-hover">
    <thead>
        <tr>
            <th>
                <a href="index.php?accion=tipos_evento&orden=id_tipo_evento&dir=<?= $orden === 'id_tipo_evento' ? $dir_toggle : 'asc' ?>&pagina=1&buscar=<?= urlencode($buscar) ?>"
                   class="text-decoration-none text-dark">
                    ID
                    <?php if ($orden === "id_tipo_evento") echo $direccion === "asc" ? "▲" : "▼"; ?>
                </a>
            </th>
            <th>
                <a href="index.php?accion=tipos_evento&orden=nombre&dir=<?= $orden === 'nombre' ? $dir_toggle : 'asc' ?>&pagina=1&buscar=<?= urlencode($buscar) ?>"
                   class="text-decoration-none text-dark">
                    Nombre
            ...
            <th>
                <a href="index.php?accion=tipos_evento&orden=activo&dir=<?= $orden === 'activo' ? $dir_toggle : 'asc' ?>&pagina=1&buscar=<?= urlencode($buscar) ?>"
                   class="text-decoration-none text-dark">
                    Estado
    ...
    <?php foreach ($tipos_pagina as $tipo) : ?>
        <tr>
            ...
            <td>
                <a href="index.php?accion=tipos_evento_ver&id=<?= $tipo["id_tipo_evento"] ?>"
                ...
                <?php if ($tipo["activo"] == 1) : ?>
                    <form action="index.php?accion=tipos_evento" method="POST"
                          onsubmit="return confirm('¿Desactivar este tipo de evento?')"
                          style="display:inline;">
                        <input type="hidden" name="accion" value="desactivar">
                        <input type="hidden" name="id"     value="<?= $tipo["id_tipo_evento"] ?>">
                        <button type="submit" class="btn btn-sm btn-danger">Desactivar</button>
                    </form>
                <?php else : ?>
                    <form action="index.php?accion=tipos_evento" method="POST"
                          onsubmit="return confirm('¿Reactivar este tipo de evento?')"
                          style="display:inline;">
